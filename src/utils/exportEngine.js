...
function buildTimesheetSheet(prep, settings, data) {
  ...
  dayEntries.forEach((day, idx) => {
    ...
    if (data.includeCheckpoints) {
      const checkpointsCol = work
        .flatMap((s) => (s.checkpoints || []).filter((cp) => !cp.private))
        .sort((a, b) => a.ts - b.ts)
        .map((cp) => `${formatTime(cp.ts)} — ${cp.text}`)
        .join("\n");
      setCell(ws, r, 11, sanitizeCell(checkpointsCol), { ...baseStyle, alignment: { wrapText: true, vertical: "center" } });
    }
    r++;
  });
  ...
  const tsWidths = [14, 7, 11, 11, 12, 12, 12, 10, 13, 24, 36];
  if (data.includeCheckpoints) tsWidths.push(40);
 ...
 */
function buildRawDataSheet(prep, data) {
  const ws = createSheet();
  ...
  prep.sessions.forEach((s, i) => {
    ...
    setCell(ws, r, colIdx, s.status, baseStyle);
    r++;
  });
  ...
  if (data.includeCheckpoints) sWidths.push(40);
  sWidths.push(10);
...
/**
 * Sheet: Work Log — standalone timestamped notes (conditional)
 */
function buildWorkLogSheet(data, cutoff) {
  const ws = createSheet();
  const entries = (data.workLog || [])
    .filter((e) => !e.private && (!cutoff || e.ts >= cutoff))
    .sort((a, b) => a.ts - b.ts);
  ...
  setTitleHeader(
    ws, 0, COLS,
    "DevTrack — Work Log",
    `${entries.length} entries  |  Generated: ${new Date().toLocaleString()}`
  );

  // Column headers
  setColumnHeaders(ws, 3, headers);

  // Data rows
  entries.forEach((e, i) => {
    const r = 4 + i;
    const style = i % 2 === 0 ? STYLES.dataCell : STYLES.dataCellAlt;
    setCell(ws, r, 0, formatDate(e.ts), style);
    setCell(ws, r, 1, formatTime(e.ts), { ...style, alignment: { horizontal: "center", vertical: "center" } });
    setCell(ws, r, 2, sanitizeCell(e.text), { ...style, alignment: { wrapText: true, vertical: "center" } });
  });

  // Footer
  const footerRow = 4 + entries.length + 1;
  setCell(ws, footerRow, 0, "Generated by DevTrack", {
    font: { name: FONT.calibri, sz: 8, italic: true, color: { rgb: COLORS.slate400 } },
    alignment: { horizontal: "center" },
  });
  ws["!merges"] = ws["!merges"] || [];
  ws["!merges"].push({ s: { r: footerRow, c: 0 }, e: { r: footerRow, c: COLS - 1 } });

  finalizeSheet(ws, [14, 10, 50], "A4", null);
  return ws;
}
...
export function generateCSVReport(data, period) {
  ...
  // Work Log section (conditional)
  if (data.includeWorkLog) {
